...
datasets = ['synthetic', 'SMD', 'SWaT', 'SMAP', 'MSL', 'WADI', 'MSDS', 'UCR', 'MBA', 'NAB', 'COLLECTOR']
...
def load_data(dataset):
	folder = os.path.join(output_folder, dataset) # /processed/COLLECTOR
	...
	if dataset == 'synthetic':
		...
		dataset_folder = 'data/COLLECTOR'
		label = pd.read_csv(os.path.join(dataset_folder, 'test_label_dmqa6.csv'))
		train = pd.read_csv(os.path.join(dataset_folder, 'train_dmqa6.csv'))
		test = pd.read_csv(os.path.join(dataset_folder, 'test_dmqa6.csv'))
		train, test = train.values.astype(float), test.values.astype(float)
		train, min_a, max_a = normalize2_1(train)
		test, _, _ = normalize2_1(test, min_a, max_a)

		label = label.values.astype(int)
		labels = np.zeros_like(test)	
		labels[label, :] = 1 # test 파일과 동일한 shape

		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file)) # folder = /processed/COLLECTOR
	else:
		raise Exception(f'Not Implemented. Check one of {datasets}')	


if __name__ == '__main__':
	# python preprocess.py COLLECTOR WADI MSL 명령 시
	# sys.argv = ['preprocess.py', 'SMD', 'WADI', 'MSL']
	commands = sys.argv[1:]
	print("commands:", commands)
	print("실행된 파일 경로:", os.path.abspath(__file__)) # __file__ : 현재 실행 중인 스크립트 파일의 경로
	load = []
	if len(commands) > 0:
		for d in commands:
			load_data(d)
