...
class Proposed_test(nn.Module):
    def __init__(self, args):

        super(Proposed_test, self).__init__()
        ...
        # self.norm_temp = nn.LayerNorm(self.enc_out)
        # self.norm_chan = nn.LayerNorm(self.enc_out)
        # self.norm_decomp = nn.LayerNorm(self.enc_out)
        self.gate = nn.Linear(self.enc_out * 3, 3)
    ...
    def forward(self, x, edge_index=None):
        ...
        # --- Fusion --- #
        stream_output = torch.cat([x_t, temp_dec_out, chan_enc_out], dim=-1) # (B, L, 3C)

        gate_weights = F.softmax(self.gate(stream_output), dim=-1)
        w_xt, w_temp, w_chan = gate_weights.split(1, dim=-1)

        x_t_gated = x_t * w_xt
        temp_dec_out_gated = temp_dec_out * w_temp
        chan_enc_out_gated = chan_enc_out * w_chan

        fused_output = torch.cat([x_t_gated, temp_dec_out_gated, chan_enc_out_gated], dim=-1)
