...
class GATv2ConvDGL(nn.Module):
    ...
    def __init__(self,
        ...
        super(GATv2ConvDGL, self).__init__()
        ...
        if isinstance(in_feats, tuple):
            ...
            self.fc_dst = nn.Linear(
                self._in_dst_feats, out_feats * num_heads, bias=True)
        else:
            self.fc_src = nn.Linear(
                self._in_src_feats, out_feats * num_heads, bias=True)
            if share_weights:
        ...
        if residual:
            if self._in_dst_feats != out_feats:
                self.res_fc = nn.Linear(
                    self._in_dst_feats, num_heads * out_feats, bias=True)
            else:
